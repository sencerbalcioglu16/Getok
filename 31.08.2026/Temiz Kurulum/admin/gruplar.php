...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($grup_adi === '' || $lig_id === 0) {
        flash_set('hata','Lig ve grup adı zorunludur.');
        redirect(BASE_URL.'/admin/gruplar.php?islem='.($_POST['islem']??'ekle').'&id='.(int)($_POST['id']??0));
    }

    if ($_POST['islem'] === 'ekle') {
        $pdo->prepare("INSERT INTO gruplar (lig_id,grup_adi,aciklama,sezon) VALUES (?,?,?,?)")
            ->execute([$lig_id, $grup_adi, $aciklama, $sezon]);
        flash_set('basari','Grup eklendi.');
    } elseif ($_POST['islem'] === 'duzenle') {
        $pdo->prepare("UPDATE gruplar SET lig_id=?, grup_adi=?, aciklama=?, sezon=? WHERE id=?")
            ->execute([$lig_id, $grup_adi, $aciklama, $sezon, (int)$_POST['id']]);
