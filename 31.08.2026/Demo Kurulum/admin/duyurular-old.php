...
// 1) ÖNCE KONFİGÜRASYON VE FONKSİYONLARI YÜKLE
require_once __DIR__ . '/../congig/config.php';
...
// 6) LAYOUT DEĞİŞKENLERİNİ TANIMLA
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($baslik === '' || trim(strip_tags($icerik)) === '') {
    ...
    if ($_POST['islem'] === 'ekle') {
        $pdo->prepare("INSERT INTO duyurular (baslik, icerik, gorsel, yayinda, yazar_id) VALUES (?,?,?,?,?)")
        ...
        $pdo->prepare("UPDATE duyurular SET baslik=?, icerik=?, gorsel=?, yayinda=? WHERE id=?")
