...
try {
    foreach(['esitlik_bozma_atislari','turnuva_sporcu_set_atislari','turnuva_mac_setleri','turnuva_maclari','turnuva_eslesmeleri','turnuva_katilimcilari','bireysel_fikstur_atislari','bireysel_fikstur','bireysel_lig_kayitlari','sporcu_set_atislari','mac_setleri','maclar','sezon_sonuclari'] as $tablo) $pdo->exec("DELETE FROM $tablo");
...
$bireyselGruplar=[];foreach(['Marmara Bölgesi','İç Anadolu Bölgesi'] as $bolge)foreach(['Minikler','Yıldızlar','Gençler','Yetişkin','Kadınlar'] as $kategori){$grupIns->execute([$bireyselLig,$bolge.' > '.$kategori,$bolge,$kategori,$kategori.' sporcuları',$sezon]);$bireyselGruplar[]=['id'=>(int)$pdo->lastInsertId(),'kategori'=>$kategori,'bolge'=>$bolge];}
...
foreach($takimAdlari as $i=>$ad){$grup=$takimGruplari[intdiv($i,4)];$logo=agent_logo(UPLOAD_DIR.'/takimlar/takim-'.($i+1).'.svg',$ad,$renkler[$i%4]);$takimIns->execute([$grup,$ad,'TOK'.($i+1),$logo,$ad.'; geleneksel okçuluk kültürünü genç sporcularla yaşatan bölge kulübüdür.',$bolgeler[intdiv($i,4)],2008+($i%12)]);$tid=(int)$pdo->lastInsertId();$takimlar[]=$tid;$takimSporcular[$tid]=[];$kategoriBilgileri=[['Minikler','2017-05-15','E'],['Yıldızlar','2013-05-15','K'],['Gençler','2009-05-15','E'],['Kadınlar','1999-05-15','K'],['Yetişkin','1995-05-15','E']];for($j=0;$j<5;$j++){$isim=$adlar[($i*5+$j)%count($adlar)];$soy=$soyadlar[($i*3+$j)%count($soyadlar)];$foto=agent_foto(UPLOAD_DIR.'/sporcular/club-'.($i*5+$j+1).'.jpg',[$fotoKaynaklari[($i+$j)%4]],$isim);[$kategori,$dogum,$cinsiyet]=$kategoriBilgileri[$j];$sporcuIns->execute([$tid,$isim,$soy,$dogum,$cinsiyet,$kategori,'CL-'.str_pad((string)($i*5+$j+1),4,'0',STR_PAD_LEFT),'05550000000','sporcu'.($i*5+$j+1).'@demo.local',$ad.' / '.$bolgeler[intdiv($i,4)],$foto]);$takimSporcular[$tid][]=['id'=>(int)$pdo->lastInsertId(),'takim_id'=>$tid];}}
