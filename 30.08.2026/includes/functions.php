...
if (!function_exists('yonetim_paneli_erisimi_var')) {
    function yonetim_paneli_erisimi_var($rol = null) {
        ...
        return in_array($rol, ['admin', 'hakem', 'yetkili', 'sporcu'], true);
    }
}
    ...
    function coklu_lig_yukselt($pdo) {
        ...
        try {
            ...
            if (!$bireysel) {
                ...
                foreach (['Marmara Bölgesi','İç Anadolu Bölgesi'] as $bolge) foreach (['Minik','Yıldız','Gençlik','Büyük','Veteran'] as $kategori) $pdo->prepare("INSERT INTO gruplar (lig_id,grup_adi,bolge_adi,kategori_adi,aciklama,sezon) VALUES (?,?,?,?,?,?)")->execute([$bireysel,$bolge.' > '.$kategori,$bolge,$kategori,'Bireysel lig bölgesi',defined('LIG_SEZON')?LIG_SEZON:null]);
            }
            ...
            foreach($eskiGruplar->fetchAll() as $eskiGrup){$bolge=trim($eskiGrup['bolge_adi']?:$eskiGrup['grup_adi']);if($bolge==='')continue;foreach(['Minik','Yıldız','Gençlik','Büyük','Veteran'] as $kategori){$varMi->execute([$bireysel,$bolge,$kategori]);if(!$varMi->fetchColumn())$olustur->execute([$bireysel,$bolge.' > '.$kategori,$bolge,$kategori,$eskiGrup['aciklama']?:'Bireysel lig bölgesi',$eskiGrup['sezon']]);}}
        } catch (PDOException $e) { }
    }
}
