...
if($_SERVER['REQUEST_METHOD']==='POST'){
 ...
 $pdo->beginTransaction(); try{
  ...
  foreach(['ev'=>$ev,'dep'=>$dep] as $taraf=>$liste)foreach($liste as $sp){$p=max(0,min(210,(int)($_POST[$taraf][$sp['id']]??0)));$in->execute([$id,$set,$taraf,$sp['id'],$p,0,0,0,0,0,0,$p]);$top[$taraf]+=$p;}
  ...
  $pdo->commit();flash_set($set===SET_SAYISI&&!$macKazanan?'hata':'basari',$set===SET_SAYISI&&!$macKazanan?'Set/maç eşit bitti; eşitlik bozma sonucu girilmeden üst tura geçilemez.':($macKazanan?'Kazanan otomatik olarak üst tura yerleştirildi.':$set.'. set kaydedildi.'));
...
ob_start(); ?>
<form method="post" class="form"><div class="card"><div class="card-head"><h2>🏆 <?= e($m['turnuva_adi']) ?></h2><span class="badge"><?= $set ?>. Set / <?= SET_SAYISI ?></span></div><p class="muted">Her sporcu için 7 okun toplam puanını tek alana girin. Maç tamamlanınca kazanan otomatik olarak bir üst turdaki yerine geçer.</p><?= csrf_field() ?><input type="hidden" name="id" value="<?= $id ?>"><input type="hidden" name="set" value="<?= $set ?>"><div class="grid-2"><?php foreach(['ev'=>$ev,'dep'=>$dep] as $taraf=>$liste): ?><section class="card"><h3><?= $taraf==='ev'?'Ev Sahibi':'Misafir' ?></h3><?php foreach($liste as $sp): ?><label class="grid-2"><span><?= e($sp['ad'].' '.$sp['soyad']) ?></span><input type="number" min="0" max="210" name="<?= $taraf ?>[<?= $sp['id'] ?>]" value="<?= (int)($mevcut[$taraf][$sp['id']]??0) ?>" placeholder="7 ok toplamı"></label><?php endforeach; ?></section><?php endforeach; ?></div><button class="btn btn-primary"><?= $set<SET_SAYISI?'Seti Kaydet ve Sonraki Sete Geç':'Son Seti Kaydet ve Maçı Tamamla' ?></button></div></form><section class="card"><h2>Set Durumu</h2><?php foreach($sets as $x): ?><p><?= $x['set_no'] ?>. Set: <?= $x['puan1'] ?> - <?= $x['puan2'] ?></p><?php endforeach; ?></section>
