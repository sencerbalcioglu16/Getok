...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['adim']) && $_POST['adim'] == 1) {
    ...
    $db_mode = $_POST['db_mode'] ?? 'existing';

    if (!preg_match('/^[A-Za-z0-9_]+$/', $name)) {
        $hata = 'Veritabanı adı yalnızca harf, rakam ve alt çizgi içerebilir.';
    }
    if (!in_array($db_mode, ['existing','create'], true)) $hata = 'Geçersiz veritabanı kurulum modu.';
    ...
    try {
        ...
        $sunucuDsn = "mysql:host={$host}" . ($port !== '' ? ";port={$port}" : '') . ";charset=utf8mb4";
        if ($db_mode === 'create') {
            $pdo0 = new PDO($sunucuDsn, $user, $pass, [PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION]);
            $pdo0->exec("CREATE DATABASE IF NOT EXISTS `{$name}` DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci");
        }
        // Paylaşımlı hostinglerde bu mod yalnızca kullanıcıya atanmış mevcut veritabanına bağlanır.
...
<div class="install-wrap">
    ...
    <form method="post" class="form">
                ...
                <div class="hint-box" style="margin-top:0">
                    <strong>Hosting kurulumu için önerilen seçenek:</strong> Önce hosting panelinizden boş bir MySQL veritabanı ve kullanıcı oluşturun; ardından aşağıdan <em>Mevcut veritabanına kur</em> seçeneğini kullanın. Bu mod, veritabanı oluşturma veya silme izni gerektirmez.
                </div>
                <div class="grid-2">
                    <label>Kurulum modu
                        <select name="db_mode">
                            <option value="existing" selected>Mevcut veritabanına kur (önerilen)</option>
                            <option value="create">Veritabanını oluşturmayı dene</option>
                        </select>
                    </label>
